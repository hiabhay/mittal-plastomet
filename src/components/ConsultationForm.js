      ...
      <div className="w-[540px] h-[550px] mt-[25px] sm:mt-0 sm:relative sm:bottom-60 sm:left-4 sm:w-11/12 bg-cyan-600 rounded-2xl">
        <form className="form pt-12 pl-10 mr-10" onSubmit={Submit}>
          ...
          >
            ...
            <div className={`text-white ml-4 mt-1 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>Submit</div>
            {!isLoading && <img src={whiteArrow} className="h-[8px] w-auto mt-[13.5px] ml-1" />}
          </button>
          {isSubmitted && (
            <p className="text-green-400 sm:text-sm mt-2">Form submitted successfully!</p>
          )}
        </form>
      </div>
    </Section>
